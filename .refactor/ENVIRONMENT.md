...
- **已修（M3-11），上一条作废**：`AppError` 现在带可选 `headers`（构造参数第 4 位，`Mapping[str, str]`），`_app_error_handler` 原样透传；M3-06 的鉴权 429 已改走 `AppError(RATE_LIMITED, headers={"Retry-After": ...})`，`StarletteHTTPException` 绕行删除。两条可复用的实测/坑：① Starlette 写响应头用 `value.encode("latin-1")`，所以头值必须是 `str`（传整数 42 会在构造 `JSONResponse` 时 AttributeError→500）且必须能编成 latin-1 —— M3-11 把它提前到 `AppError` 构造期 fail loud；② `BaseException` 默认 `__reduce__` 依赖 `self.args` 位置重建，给异常加第 4 个位置参数并在 `super().__init__` 里传归一后的纯 dict，pickle 往返（含 headers）无需自定义 `__reduce__`。
...
### M4-01 实测：WS 路由/关闭码、跨线程入队、IPC 事件载荷、asst.log tail 陷阱（第 27 次尝试追加）

完整产物：`scripts/probe_log_ws.py`、`tests/fixtures/log_ws_probe_result.json`、`tests/fixtures/log_ws_probe_findings.md`
（11 条 required_checks 全 true，`probe_ok=True`，实测 exit 0，stderr 干净；全部实验在临时目录/进程内跑，
不联网、不碰真实 `resource/maa_api.db` 与 `config.yaml`——探针自带 env guard 复核）。环境：FastAPI 0.141.1 /
Starlette 1.6.0 / httpx 0.27.2 / Python 3.13.3。

- **`from __future__ import annotations` 下把 `WebSocket` 只在函数内 import，FastAPI 会把 `websocket: WebSocket` 当必填 query 参数**（M4-01 实测）：路由能注册成功、`router.routes[*].path` 也正确，但每次握手都以 `close(1008)` 结束，`WebSocketDisconnect.reason` 里是 pydantic 的 `loc=["query","websocket"] Field required`。FastAPI 解析端点注解走**模块全局命名空间**，注解名必须在模块级可见（正常写 `from fastapi import WebSocket` 在模块顶部就没这问题）。探针第一版就是踩这个坑，改了模块级 import 才拿到 4401/1000。
- **WS 路由的可发现性（决定 WS 卡 verify 怎么写）**：`app.openapi()['paths']` 只有 HTTP 路由；`fastapi.routing.iter_route_contexts(app.routes)` 对 `include_router` 进来的 WS 路由只给 `path=''` 的上下文对象（实测 `type(...).__name__ == 'RouteContext'`、`has_tags=True`、`name=''`，一条 include 里出现 2 条空 path 上下文）；`app.routes` 里的 `_IncludedRouter` 没有 `.path`（M3 已记）。**唯一可靠的是模块级 `<ws 模块>.router.routes[*].path`**：推荐 verify 写成 `sorted(r.path for r in maa_api.api.ws.router.routes if isinstance(r, fastapi.routing.APIWebSocketRoute))`。`_IncludedRouter.original_router.routes` 能挖出路径但是私有属性，不要写进门禁。
- **TestClient 的 WS 关闭码语义**（实测）：`accept()` 后 `close(4401)` → `WebSocketDisconnect.code == 4401`；**不存在的 WS 路径 → 同样抛 `WebSocketDisconnect` 但 code == 1000**；把**已注册的 HTTP 路径**当 WS 连也是 1000。所以「抛异常」与 code=1000 都不能证明路由存在/缺失，路由存在性只能静态断言模块级 `router.routes`。另：服务端在 `accept()` 之前 `close(1008)` 时 TestClient 侧**能读到 1008**（与真机浏览器只剩 HTTP 403 不同），别把这条写进跨环境断言。
- **`asyncio.Queue.put_nowait` 跨线程不唤醒事件循环**（实测，决定 `LogHub.offer` 的写法）：事件循环在另一条线程里 `await queue.get()`、没有其它定时器/IO 时，主线程裸 `put_nowait` 在 500ms 窗口内消费者收不到（条目没丢，之后任一 `call_soon_threadsafe` 唤醒即送达）；改用 `loop.call_soon_threadsafe(queue.put_nowait, item)` 实测延迟 0.1–0.5ms。若 `loop.set_debug(True)`，裸 put 直接在生产者线程抛 `RuntimeError: Non-thread-safe operation invoked on an event loop other than the current one`，且 getter future 的 callback 永远不会被调度（条目对消费者永久不可见）。`asyncio.Queue` 不是线程安全的，跨线程投递只能走 `call_soon_threadsafe`。
- **`CoreClient` 分派给 `on()` handler 的是 payload，不含 envelope `ts`**（实测，M1 契约）：LOG 的键集 `{level, content}`、CALLBACK 的 `{msg, details}`；只有 `supervisor.note_event` 收到 `{type, ts, payload}`。M4 的 `LogRecord.ts` 只能取 handler 内的到达时刻（除非改 `CoreClient` 的公开签名，属 M1 变更）。
- **`asst.log` tail：docs/06 §5.4 的半行回退写法有字节数陷阱**（实测）：`errors="replace"` 下，含非法 UTF-8 字节的半行重新 `len(line.encode("utf-8"))` 会比实际读入的字节数**大**（U+FFFD 编 3 字节），照抄 `fp.seek(fp.tell() - len(line.encode("utf-8")))` 会多退到前一行的中间（下一次读到重复片段，实测 `reread="1\n"`），在文件头部直接 `ValueError: negative seek position -2`（不是 OSError）。正确做法：读之前记 `position_before = fp.tell()`，半行时 `fp.seek(position_before)`。其余语义实测成立：首次打开 seek 到末尾不回放历史；`os.rename` 轮转后新文件 `st_ino` 变化、旧句柄仍能把 rename 前写入的尾部读到 EOF；截断只改 `st_size` 不改 `st_ino`，用 `st_size < 上次 offset` 判。
- **本机没有任何可用的真实 WS 实现**（实测，影响 M4 冒烟与 `--serve`）：`.venv` 里 `websockets` / `wsproto` / `websocket`(websocket-client) / `httpx_ws` / `aiohttp` 全部 import 失败，`websocat`/`wscat` 不存在（`wsdump` 属于系统 Python 的 site-packages，`.venv` 用不了）；uvicorn 0.53.0 的 `uvicorn.protocols.websockets.auto.AutoWebSocketsProtocol is None`，真实 uvicorn 对 WS 升级回 `HTTP/1.1 404 Not Found` 并打日志 `WARNING: Unsupported upgrade request.` + `WARNING: No supported WebSocket library detected. Please use "pip install 'uvicorn[standard]'", or install 'websockets' or 'wsproto' manually.`。因此 WS 冒烟默认模式只能用 `fastapi.testclient.TestClient`（进程内 ASGI）；要让 `--serve` 真能升级 WS，必须先把 `websockets`（或 `wsproto`）加进依赖。
- **TestClient 的 WS 握手携带 cookie 的方式**（实测）：`client.websocket_connect(url, headers={"cookie": "maa_token=..."})` 与 per-request `cookies={...}` 都能进 `WebSocket` scope 的 cookies；`client.cookies.set(..., domain="testserver")`（client 级 cookie jar）**不会**进 WS 握手。
- **复跑注意**：`.venv/bin/python scripts/probe_log_ws.py` 每次都会覆盖 `tests/fixtures/log_ws_probe_result.json` 与 `log_ws_probe_findings.md`（含时间戳、临时端口、inode 等不稳定字段），需要稳定内容的场景不要直接 diff 这两个文件；探针里有一段真实 uvicorn 子进程 + 127.0.0.1 临时端口的裸 socket 握手，整脚本约 5s。
