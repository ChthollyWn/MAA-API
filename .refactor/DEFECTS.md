...
- [ ] M2-05 | 旧 `weekday_task` 的 Python weekday（0=周一…6=周日）被 0002 原样写入 POSIX cron（0=周日），迁移后的七个按星期任务会整体错一天 | 复现：`.venv/bin/python -m pytest -q tests/db/test_daily_task_migration.py::test_upgrade_expands_every_weekday` 并对照 `maa_api/scheduler/daily_task_scheduler.py::daily_art_task` 的 `date.today().weekday()` 与 `maa_api/db/migrations/versions/0002_migrate_daily_task_json.py::iter_rows` 的 cron 直接插值 | 发现于 M9-03；修复卡 M9-08
