  ...
  return <div className="space-y-3">
    ...
    <ul className="space-y-2">{fixedLevels.map((key) => {
      ...
      return <li key={key} className="grid grid-cols-[minmax(0,1fr)_5rem_5rem_2.75rem] items-end gap-2 rounded-lg border p-2">
        ...
        <Button type="button" size="sm" variant="ghost" aria-label={`恢复${key}星 Tag 默认时长`} disabled={disabled || (value[key] === undefined && minutes === 540)} onClick={() => onChange({ ...value, [key]: 540 })}>恢复默认</Button>
      </li>
    })}</ul>
...
function ValueEditor({
  ...
  const stateSelector = isNullable ? (
    <div className="mb-2 flex flex-wrap gap-2">
      <Button type="button" size="sm" variant={value === undefined ? 'secondary' : 'outline'} disabled={disabled} onClick={() => onChange(UNSET)}>
        使用内核默认
      </Button>
      <Button type="button" size="sm" variant={value === null ? 'secondary' : 'outline'} disabled={disabled} onClick={() => onChange(null)}>
        明确留空
      </Button>
    </div>
  ) : null
      ...
      return <div className="space-y-3">
        ...
        {widget === 'tags' ? <div className="flex flex-wrap gap-2">
          {itemOptions.map((option) => {
            ...
            return <Button key={JSON.stringify(option)} type="button" size="sm" variant={active ? 'secondary' : 'outline'} disabled={disabled || (!active && values.length >= maxItems)} onClick={() => onChange(active ? values.filter((entry) => !Object.is(entry, option)) : [...values, option])} aria-pressed={active}>
          ...
          <div className="flex flex-wrap gap-2">{itemOptions.filter((option) => !values.some((entry) => Object.is(entry, option))).map((option) => <Button key={JSON.stringify(option)} type="button" size="sm" variant="outline" disabled={disabled || values.length >= maxItems} onClick={() => addOption(option)}><Plus aria-hidden="true" />{enumLabels(normalizedItem.schema)[String(option)] ?? String(option)}</Button>)}</div>
        </>}
        {error && <p id={`${id}-error`} className="text-sm text-destructive" role="alert">{error}</p>}
      </div>
    }
...
function collectFieldIssues(body: unknown, taskCount: number): { issues: FieldIssue[]; message: string } {
  ...
  const error = unwrapHttpError(body)
...
function pipelineTitle(row: PipelineRow): string {
  return textValue(row.title, '') || pipelineId(row) || '未命名流水线'
}
...
export default function TasksFeature() {
  ...
  const [activeView, setActiveView] = useState<'create' | 'queue' | 'history'>('create')
  ...
  const typeQuery = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await api.GET('/api/tasks/types')
      if (error) throw error
  ...
  const queueQuery = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await api.GET('/api/queue')
      if (error) throw error
  ...
  const historyQuery = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await api.GET('/api/pipelines', { params: { query: { page: historyPage, size: historyPageSize } } })
      if (error) throw error
  ...
  const selectedPipelineQuery = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await api.GET('/api/pipelines/{pipeline_id}', { params: { path: { pipeline_id: selectedPipelineId ?? '' } } })
      if (error) throw error
  ...
  const selectedPipelineLogsQuery = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await api.GET('/api/pipelines/{pipeline_id}/logs', {
  ...
  const selectedPipelineScreenshotsQuery = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await api.GET('/api/pipelines/{pipeline_id}/screenshots', {
  ...
  return <main className="mx-auto w-full max-w-5xl space-y-5 px-4 pb-24 pt-5 sm:px-6">
    ...
    <div className="grid grid-cols-3 gap-2 rounded-xl border bg-muted/40 p-1" role="tablist" aria-label="任务视图">
      {([['create', '创建'], ['queue', '队列'], ['history', '历史']] as const).map(([view, label]) => <Button key={view} type="button" variant={activeView === view ? 'default' : 'ghost'} role="tab" aria-selected={activeView === view} onClick={() => setActiveView(view)}>{label}</Button>)}
    </div>
    ...
    {activeView === 'create' && <section role="tabpanel" aria-label="创建任务" className="space-y-4">
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {typeQuery.error && <div className="rounded-lg border border-destructive/40 bg-destructive/5 p-3 text-sm text-destructive" role="alert">无法读取任务类型：{errorMessage(typeQuery.error)} <Button type="button" size="sm" variant="outline" onClick={() => void typeQuery.refetch()}>重试</Button></div>}
          ...
          <div className="space-y-2"><Label htmlFor="pipeline-title">流水线名称（可选）</Label><Input id="pipeline-title" maxLength={64} value={title} onChange={(event) => setTitle(event.target.value)} placeholder="例如：每日基建与刷图" /></div>
            ...
            {drafts.map((draft, index) => {
              ...
              return <li key={draft.id} className="rounded-xl border bg-card">
                <div className="flex flex-wrap items-center gap-2 border-b px-3 py-2 sm:px-4">
                  ...
                  <Button type="button" size="icon" variant="ghost" aria-label={`删除${taskType.label}`} disabled={submitMutation.isPending} onClick={() => setDrafts((current) => current.filter((item) => item.id !== draft.id))}><Trash2 aria-hidden="true" /></Button>
                </div>
          ...
          <div className="flex flex-col gap-3 border-t pt-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Button type="button" onClick={submitPipeline} disabled={drafts.length === 0 || drafts.length > 32 || submitMutation.isPending || typeQuery.isLoading}>
              {submitMutation.isPending ? <><LoaderCircle className="animate-spin" aria-hidden="true" />正在校验并提交…</> : '校验并加入队列'}
            </Button>
          </div>
          ...
          {queueQuery.isLoading ? <p role="status" className="text-sm text-muted-foreground">正在读取队列…</p> : pending.length === 0 ? <div className="rounded-xl border border-dashed p-6 text-center text-sm text-muted-foreground">当前没有待执行条目。</div> : <ol className="space-y-3" aria-label="服务端待执行顺序">
            {pending.map((row, index) => <li key={pipelineId(row) || index} className="rounded-xl border p-4">
              ...
              <div className="mt-3 flex flex-wrap gap-2 border-t pt-3">
                <Button type="button" size="sm" variant="outline" disabled={queueMutation.isPending || !pipelineId(row)} onClick={() => {
                  if (window.confirm(`取消待执行流水线「${pipelineTitle(row)}」？`)) queueMutation.mutate({ kind: 'cancel', pipelineId: pipelineId(row) })
                }}>取消此条</Button>
                <Button type="button" size="sm" variant="destructive" disabled={queueMutation.isPending || !queue?.running || !pipelineId(row)} onClick={() => void stopAndPromote(row)}>停止当前并执行此条</Button>
              </div>
            </li>)}
          </ol>}
        </CardContent>
            ...
            {history?.items.map((row) => {
              ...
              return <li key={id} className="rounded-xl border">
                <button type="button" className="flex min-h-16 w-full flex-wrap items-center gap-3 p-4 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" aria-expanded={selected} onClick={() => {
                  setSelectedPipelineId(selected ? null : id)
                  setHistoryStatus('')
                }}>
                ...
                {selected && <div className="space-y-4 border-t p-4">
                  ...
                  {detail && pipelineId(detail) === id && <>
                    ...
                    <div className="flex flex-wrap gap-2 border-t pt-3">
                      {(detail.status === 'PENDING' || detail.status === 'RUNNING' || detail.status === 'pending' || detail.status === 'running') && <Button type="button" size="sm" variant="destructive" disabled={historyAction.isPending} onClick={() => {
                        if (window.confirm(`请求取消流水线「${pipelineTitle(detail)}」？`)) historyAction.mutate({ kind: 'cancel', pipelineId: id })
                      }}>取消流水线</Button>}
                      {(detail.status === 'FAILED' || detail.status === 'CANCELLED' || detail.status === 'failed' || detail.status === 'cancelled') && <Button type="button" size="sm" variant="outline" disabled={historyAction.isPending} onClick={() => {
