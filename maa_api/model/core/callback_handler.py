...
class CallbackHandler:
    ...
    def handle_message(self, msg, details):
        """回调主处理函数"""
        try:
            ...
            if message_type == Message.ConnectionInfo:
                self._handle_connection_info(d)
            elif message_type in [Message.TaskChainStart, Message.TaskChainCompleted, Message.TaskChainError]:
                self._handle_task_status(message_type, d)
            elif message_type == Message.SubTaskStart:
                self._handle_subtask_start(d)
            elif message_type == Message.SubTaskExtraInfo:
                self._handle_subtask_extra_info(d)
            elif message_type == Message.AllTasksCompleted:
                self._handle_all_tasks_completed()

        except Exception as e:
            logger.error(f"回调处理异常: {e}", exc_info=True)

    def _handle_connection_info(self, details: Dict[str, str]):
        """处理连接状态变化"""
    ...
    def _handle_task_status(self, msg: Message, details: Dict[str, str]):
        """处理任务状态变化"""
        task_id = details.get('taskid')
        taskchain = details.get('taskchain')
        task = self.pipeline._task_dict.get(task_id)
        ...
        self.pipeline.update_task_log(task_id, log)
        logger.info(log)

    def _handle_subtask_start(self, d: Dict[str, Any]):
        """处理子任务开始"""
        ...
        if sub_task in sub_task_info:
            ...
            self.pipeline.logs.append(f'{_current_time()} {log}')
            self._log(log)

    def _handle_subtask_extra_info(self, d: Dict[str, Any]):
        """处理子任务额外信息"""
        sub_what = d.get('what', '')
        sub_details = d.get('details', {})
    ...
    def _handle_all_tasks_completed(self):
        """处理所有任务完成"""
        self.pipeline.status = TaskPipelineStatus.COMPLETED
        log = '已完成全部任务'
        self.pipeline.logs.append(f'{_current_time()} {log}')
        self._log(log)
    ...
    def _log(self, log: str):
        """日志记录"""
        from maa_api.model.core.pipeline import TASK_PIPELINE_LOG_DIR
        current_date = _current_time()
        file_path = TASK_PIPELINE_LOG_DIR / f'task_pipeline_{current_date}.log'
        with file_path.open('a', encoding='utf-8') as f:
            f.write(f'{_current_datetime()} {log}\n')
        logger.info(log)
...
def callback(msg, details, arg):
    ...
    callback_handler.handle_message(msg, details)
